...
__global__ void Effet_diapositive(unsigned int* source, unsigned int* destination, unsigned width, unsigned height)
{
  int colonne,ligne;
...
{ 
  // for each pixel color value c, c = 255 -c
  destination[((ligne * width + colonne) * 3) + 0] = 255-source[((ligne * width + colonne) * 3) + 0];
  destination[((ligne * width + colonne) * 3) + 1] = 255-source[((ligne * width + colonne) * 3 )+ 1];
  destination[((ligne * width + colonne) * 3) + 2] = 255-source[((ligne * width + colonne) * 3 )+ 2];
}
...
int main (int argc , char** argv)
{
  ...
  // kernel applys negative affect to the image
  Effet_diapositive<<<gridSize, blockSize>>>(cuda_source, cuda_destination, width, height);
