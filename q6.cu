...
__global__ void satur(unsigned int* source, unsigned int* destination, unsigned width, unsigned height)
{
  int colonne,ligne;
...
{
  // saturate red, leave other green and blue as source
  destination[((ligne * width + colonne) * 3) + 0] = 255;
  destination[((ligne * width + colonne) * 3) + 1] = source[((ligne * width + colonne) * 3 )+ 1];
  destination[((ligne * width + colonne) * 3) + 2] = source[((ligne * width + colonne) * 3 )+ 2];
}
...
int main (int argc , char** argv)
{
  ...
  satur<<<gridSize, blockSize>>>(cuda_source, cuda_destination, width, height);
